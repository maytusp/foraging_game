...
class Args:
    ...
    grid_size = 13
    max_walls = grid_size
    timesteps_per_wall = 1 # 1 means use max_walls #  int(0.9 * (total_timesteps // max_walls)) # max_walls at 80% of the training time
    # Fraction of training (0.0 to 1.0) when the Easy Phase (Mode 0) ends
    curriculum_easy_end: float = 0.0
    curriculum_medium_end: float = 0.0
    max_steps = 30
    ...
    if load_pretrained:
        pretrained_global_step = 999997440
        learning_rate = 2e-4
        print(f"LOAD from {pretrained_global_step}")
        ckpt_path = {}
        for a in range(num_networks):
            ckpt_path[a] = f"checkpoints/torch_scoreg_scale/lstm2k_sp_ppo_2net_invisible_wospeedrw/grid13_img7_ni2_nw4_ms30_nwall13/seed{seed}/agent_{a}_step_{pretrained_global_step}.pt"
