...
# CUDA_VISIBLE_DEVICES=1 python -m scripts.pickup_high_v7.train_il_reset_no_comm_all_visible
@dataclass
class Args:
    seed: int = 1
    ...
    ablate_message = True
    mode = "train"
    model_name = "dec_ppo_il"
    il_reset_every: int = (1e7 // (num_envs * num_steps)) # reset every 10M env steps
    il_stop_reset_after: int = (6e7 // (num_envs * num_steps)) # stop reset after 60M env steps
    
    if not(agent_visible):
        model_name+= "_invisible"
    if ablate_message:
        model_name += "_ablate_message"
    if fully_visible_score:
        model_name += "_visible_score"
    ...
    if load_pretrained:
        pretrained_global_step = 409600000
        learning_rate = 2e-4
    visualize_loss = True
    ckpt_path = {
                0:f"checkpoints/pickup_high_v7/dec_ppo/grid5_img5_ni2_nw4_ms10/seed1/agent_0_step_409600000.pt", 
                1:f"checkpoints/pickup_high_v7/dec_ppo/grid5_img5_ni2_nw4_ms10/seed1/agent_1_step_409600000.pt"
                }
...
if __name__ == "__main__":
    ...
    print(f"IL RESET EVERY {args.il_reset_every}")
    print(f"IL STOP RESET AFTER {args.il_stop_reset_after}")
    if args.track:
        import wandb
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # [IL] Iterated Learning: reset heads every N iterations; log fingerprints to W&B and TensorBoard
        do_il_reset = (
            args.il_reset_every > 0
            and (iteration % args.il_reset_every == 0)
            and (iteration <= args.il_stop_reset_after)
        )
        if do_il_reset:
            for i in range(num_agents):

                # -- (B) RESET HEADS
                agents[i].reset_actor_critic()
                reset_params = list(agents[i].actor.parameters()) + list(agents[i].critic.parameters())

                # -- (C) CLEAR ADAM STATE ONLY FOR RESET PARAMS
                opt_state = optimizers[i].state
                for p in reset_params:
                    if p in opt_state:
                        del opt_state[p]


                # TensorBoard marker (1 at reset steps)
                writer.add_scalar(f"agent{i}/il/reset_flag", 1, global_step)
