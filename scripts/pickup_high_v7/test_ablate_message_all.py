...
# python -m scripts.pickup_high_v7.test_ablate_message_all
...
class Args:
    ...
    model2step = {
                # "dec_ppo_invisible/grid5_img3_ni2_nw4_ms10" : 307200000,
                "dec_ppo_invisible_ablate_message/grid5_img3_ni2_nw4_ms10":307200000,
                "dec_ppo_ablate_message/grid5_img3_ni2_nw4_ms10":307200000,
                "dec_ppo_ablate_message/grid5_img5_ni2_nw4_ms10":307200000,
                }
    

if __name__ == "__main__":
    args = tyro.cli(Args)
    for test_moderate_score in [True, False]:
        args.test_moderate_score = test_moderate_score
        
        for model_comb, model_step in args.model2step.items():
            model_name, combination_name = model_comb.split("/")
            args.model_name = model_name
            args.model_step = model_step
            args.combination_name = combination_name
            if "img3" in combination_name:
                args.image_size = 3
            elif "img5" in combination_name:
                args.image_size = 5

            if "_invisible" in model_name:
            ...
            for seed in [1,2,3]:
                ...
                args.saved_dir = f"logs/ablate_message_during_train/pickup_high_v7/{args.model_name}/{network_pairs}/{args.combination_name}_{args.model_step}/seed{args.seed}/mode_{args.mode}"

                if args.test_moderate_score:
                    args.saved_dir = os.path.join(args.saved_dir, "moderate_"+args.ablate_type)
                else:
                        ...
                        with torch.no_grad():
                            if args.ablate_message:
                                if args.ablate_type == "zero":
                                    ...
                                    next_r_messages = torch.randint(0, args.n_words, next_r_messages.shape).to(device)
                                else:
