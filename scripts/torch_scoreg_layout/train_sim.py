# Update multiple (if not all) networks in one iteration
# The original code only updates a pair of networks per iteration
# Note: This code doesn't accelerate learning so far. We can use the original one.
import os
import random
import time
...
from utils.process_env import assign_pairs_per_env
from models.pickup_models import PPOLSTMCommAgent

import math

# CUDA_VISIBLE_DEVICES=0 python -m scripts.torch_scoreg_layout.train_sim --seed 1 --comm_field 100 --num_networks 3 --no-agent-visible
@dataclass
class Args:
    seed: int = 1
...
if __name__ == "__main__":
    ...
    model_name = f"sim_{sp_prefix}pop_ppo_{args.num_networks}net"
    ...
    global_step = 0
    initial_lstm_state = {}
    possible_pairs = [[a,b] for a in range(args.num_networks) for b in range(args.num_networks) if a != b]
    if args.self_play_option:
