...
# Example:
# CUDA_VISIBLE_DEVICES=0 python -m scripts.torch_scoreg_layout.eval_batched_sr_mappo --num-networks 2 --seed 1 --all-pairs --model-step 486400000 --batch-num-envs 1024
...
from models.pickup_models import PPOLSTMCommActor
...
class Args:
    ...
    num_networks: int = 2
    model_name: str = "pop_mappo_2net_invisible"
...
def resolve_model_name(args: Args) -> str:
    ...
    model_name = f"pop_mappo_{args.num_networks}net"
...
def build_agent(args: Args, device: torch.device, num_actions: int, num_channels: int) -> PPOLSTMCommActor:
    return PPOLSTMCommActor(
...
def load_population(
    ...
    for agent_id in range(args.num_networks):
        ckpt_path = (
            ...
            f"seed{args.seed}/actor_{agent_id}_step_{args.model_step}.pt"
        )
...
def evaluate_pair_sr_batched(
    sender_agent: PPOLSTMCommActor,
    receiver_agent: PPOLSTMCommActor,
    args: Args,
    device: torch.device,
    ascii_layout,
) -> tuple[float, float]:
    ...
    while finished_episodes < args.total_episodes:
        ...
        action0, _, _, s_message0, _, _, next_lstm_state0 = sender_agent.get_action_and_logprob(
            (
                next_obs[:, 0, :, :, :],
                next_locs[:, 0, :],
                next_r_messages[:, 0],
        ...
        action1, _, _, s_message1, _, _, next_lstm_state1 = receiver_agent.get_action_and_logprob(
            (
                next_obs[:, 1, :, :, :],
                next_locs[:, 1, :],
                next_r_messages[:, 1],
