...
# CUDA_VISIBLE_DEVICES=1 python -m scripts.pickup_rg.train_pop_3net_ft
class Args:
    seed: int = 1
    ...
    if load_pretrained:
        pretrained_global_step = 204800000
        learning_rate = 2e-4
        print(f"LOAD from {pretrained_global_step}")
        ckpt_path = {
                    a: f"checkpoints/pickup_rg/pop_ppo_6net_invisible/grid5_img3_ni2_nw4_ms6/seed1/agent_{a}_step_204800000.pt" for a in range(num_networks)
                    }
    visualize_loss = True
