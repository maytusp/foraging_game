...
# CUDA_VISIBLE_DEVICES=1 python -m scripts.pickup_high_v1.train_sp_pop_12net_seed3
@dataclass
class Args:
    seed: int = 3
    ...
    agent_visible = False
    mode = "train"
    model_name = f"pop_sp_ppo_{num_networks}net"
    ...
    if load_pretrained:
        pretrained_global_step = 947200000
        learning_rate = 2e-4
        print(f"LOAD from {pretrained_global_step}")
        ckpt_path = {
                    a: f"checkpoints/pickup_high_v1/pop_ppo_15net_invisible/grid5_img3_ni2_nw4_ms10/seed1/agent_{a}_step_947200000.pt" for a in range(num_networks)
                    }
    visualize_loss = True
