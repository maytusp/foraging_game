# Training Recurrent Memory Transformer (RMT) in CPU environment
import os
import random
import time
...
from models.pickup_models import PPOTransformerCommAgent  # Using RMT Agent

# CUDA_VISIBLE_DEVICES=1 python -m scripts.pickup_high_v1.train_rmt_ring
...
class Args:
    ...
    agent_visible = False
    mode = "train"
    model_name = f"rmt_ring_sp_ppo_{num_networks}net"
    ...
    save_dir = f"checkpoints/pickup_cpu_rmt/{model_name}/{train_combination_name}/seed{seed}/"
    ...
    if load_pretrained:
        ...
        ckpt_path = {
            a: f"checkpoints/pickup_cpu_rmt/{model_name}/{train_combination_name}/seed1/agent_{a}_step_{pretrained_global_step}.pt"
...
if __name__ == "__main__":
    ...
    next_done, next_memory_state = {}, {}
    
    # TRY NOT TO MODIFY: start the game
    next_obs_dict, _ = envs.reset(seed=args.seed)

    # --- Create Population of RMT Agents ---
    ...
    # --- Storage Setup (Using RMT Memory logic) ---
    for i in range(num_agents):
        ...
        # Initial Observation Extraction from CPU Env
        next_obs[i], next_locs[i], next_r_messages[i], _ = extract_dict_separate(
            next_obs_dict, None, device, i, num_agents, use_message=True
        )
        
        # Ensure tensors are on device
        next_obs[i] = torch.Tensor(next_obs[i]).to(device)
        next_locs[i] = torch.Tensor(next_locs[i]).to(device)
        next_r_messages[i] = torch.tensor(next_r_messages[i]).squeeze().to(device)
        next_done[i] = torch.zeros(args.num_envs).to(device)
        
        # Initialize RMT Memory State: [num_envs, d_model]
    ...
    if args.self_play_option:
        ...
        print(f"SELF-PLAY ENABLED with POSSIBLE PAIRS = \n : {possible_pairs}")
    ...
    for iteration in range(1, args.num_iterations + 1):
        if iteration % args.reset_iteration == 0:
            # Reset memory state when resampling networks
            for i in range(num_agents):
                next_memory_state[i] = agents[0].init_memory(args.num_envs, device)
        ...
        for step in range(0, args.num_steps):
            ...
            next_obs_dict, all_reward, all_terminations, all_truncations, infos = envs.step(
                {"action": env_action, "message": env_message}
            )
            env_info = (all_reward, all_terminations, all_truncations)

            # Log experience from the environment
            for i in range(num_agents):
                next_obs[i], next_locs[i], next_r_messages[i], (reward[i], terminations, truncations) = extract_dict_separate(
                    next_obs_dict, env_info, device, i, num_agents, use_message=True
                )

                next_done[i] = np.logical_or(terminations, truncations)
                rewards[i][step] = torch.tensor(reward[i]).to(device).view(-1)
                
                # Convert back to tensors for next step
                next_obs[i] = torch.Tensor(next_obs[i]).to(device)
                next_locs[i] = torch.Tensor(next_locs[i]).to(device)
                next_r_messages[i] = torch.tensor(next_r_messages[i]).to(device)
                next_done[i] = torch.Tensor(next_done[i]).to(device)
        ...
        for i in range(num_agents):
            ...
            for epoch in range(args.update_epochs):
                ...
                for start in range(0, args.num_envs, envsperbatch):
                    ...
                    # RMT Memory retrieval for minibatch
            ...
            if args.visualize_loss and (global_step // args.num_envs) % args.log_every == 0:
                ...
                writer.add_scalar(
                    f"agent{network_id}/losses/explained_variance",
                    explained_var,
                    global_step,
                )
                writer.add_scalar(
