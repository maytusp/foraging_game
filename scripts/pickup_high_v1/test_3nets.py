        ...
        for seed in [1]:
            for i in range(args.num_networks):
                for j in range(i+1):
                    ...
                    network_pairs = f"{i}-{j}"
                    selected_networks = network_pairs.split("-")
                    if args.model_step == "final":
                        first_agent_net = f"final_model_agent_{selected_networks[0]}.pt"
                        second_agent_net = f"final_model_agent_{selected_networks[1]}.pt"
                    else:
                        first_agent_net = f"agent_{selected_networks[0]}_step_{args.model_step}.pt"
                        second_agent_net = f"agent_{selected_networks[1]}_step_{args.model_step}.pt"

                    ckpt_path = f"checkpoints/pickup_high_v1/{args.model_name}/{args.combination_name}/seed{seed}/{first_agent_net}"
                    ckpt_path2 = f"checkpoints/pickup_high_v1/{args.model_name}/{args.combination_name}/seed{seed}/{second_agent_net}"
                    saved_dir = f"logs/pickup_high_v1/{args.model_name}/{network_pairs}/{args.combination_name}_{args.model_step}/seed{seed}/mode_{args.mode}"
                    
                    if args.ablate_message:
                        saved_dir = os.path.join(saved_dir, args.ablate_type)
                    else:
                        saved_dir = os.path.join(saved_dir, "normal")
                            
                    run_name = f"{args.env_id}__{args.exp_name}__{seed}__{int(time.time())}"
                    print(f"saved at: {saved_dir}")
                    ...
                    agent0.load_state_dict(torch.load(ckpt_path, map_location=device))
                    agent0.eval()
                    ...
                    agent1 = PPOLSTMCommAgent(num_actions=num_actions, 
                                                ...
                                                num_channels=num_channels, 
                                                image_size=args.image_size).to(device)
                    agent1.load_state_dict(torch.load(ckpt_path2, map_location=device))
                    agent1.eval()
                    ...
                    for episode_id in range(1, args.total_episodes + 1):
                        ...
                        if not(args.save_trajectory):
                            ...
                            with open(os.path.join(saved_dir, "log.txt"), "a") as log_file:
                    ...
                    if args.save_trajectory:
                        with open(os.path.join(saved_dir, "trajectory.pkl"), "wb") as f:
                            pickle.dump(log_data, f)

                    with open(os.path.join(saved_dir, "score.txt"), "a") as log_file:
