...
# CUDA_VISIBLE_DEVICES=1 python -m scripts.pickup_temporal.train_8x8_ablate_3net_inv_longer
@dataclass
class Args:
    seed: int = 1
    ...
    if load_pretrained:
        pretrained_global_step = 601600000
        learning_rate = 2e-4
        print(f"LOAD from {pretrained_global_step}")
        ckpt_path = {
                    a: f"checkpoints/pickup_temporal/pop_ppo_3net_invisible_ablate_message/grid8_img3_ni2_nw4_ms40_freeze_dur6/seed1/agent_{a}_step_601600000.pt" for a in range(num_networks)
                    }
    visualize_loss = True
