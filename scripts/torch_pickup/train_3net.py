...
# python -m scripts.torch_pickup.train_3net
@dataclass
class Args:
    seed: int = 1
...
if __name__ == "__main__":
    ...
    # rolling windows
    recent_returns = deque(maxlen=100)   # last N completed episodes (team-mean)
    ...
    LOG_EVERY_EPISODES = getattr(args, "log_every_episodes", 100)  # tune as you like
    # Start training
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if finished.any():
                ...
                recent_returns.extend(finished_returns)
                recent_lengths.extend(finished_lengths)
                episodes_since_log += len(b_idx)
            ...
            if args.visualize_loss and episodes_since_log >= LOG_EVERY_EPISODES and len(recent_returns) > 0:
                ...
                writer.add_scalar("charts/episodic_return/mean_lastN", mean_ret, global_step)
                writer.add_scalar("charts/episodic_length/mean_lastN", mean_len, global_step)
