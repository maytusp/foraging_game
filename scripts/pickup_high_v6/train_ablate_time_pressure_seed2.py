# Created: 28 Feb 2025
# The code is for training agents with separated networks during training and execution (no parameter sharing) for pickup_high_v6
...
# CUDA_VISIBLE_DEVICES=1 python -m scripts.pickup_high_v6.train_ablate_time_pressure_seed3
@dataclass
class Args:
    seed: int = 2
    ...
    if load_pretrained:
        global_step = 204800000
        learning_rate = 2e-4
    visualize_loss = True
    ckpt_path = {
                0:f"checkpoints/pickup_high_v6/add_time_pressure/seed2/agent_0_step_204800000.pt", 
                1:f"checkpoints/pickup_high_v6/add_time_pressure/seed2/agent_1_step_204800000.pt"
                }
    ...
    wandb_project_name: str = "pickup_high_v6"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for i in range(num_agents):
            ...
            if args.visualize_loss and (global_step // args.num_envs) % args.log_every == 0:
                ...
                writer.add_scalar("charts/SPS", int(sps_step / (time.time() - start_time)), sps_step)
