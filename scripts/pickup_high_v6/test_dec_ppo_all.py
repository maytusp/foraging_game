...
from environments.pickup_high_v6 import *
from utils.process_data import *
from models.pickup_models import PPOLSTMCommAgent


# CUDA_VISIBLE_DEVICES=1 python -m scripts.pickup_high_v6.test_dec_ppo_all
# 
@dataclass
class Args:
    ...
    model2step = {#"normal_condition":563200000,
                    #"add_time_pressure":563200000, 
                            # "add_invisible":563200000,
                            # "increase_complexity":563200000,
                            "reduce_receptive_field":563200000,
                            }
if __name__ == "__main__":
    args = tyro.cli(Args)
    
    # Loop over all network pair combinations (0-0, 0-1, …, 2-2)
    for model_name in args.model2step.keys():
        args.model_name = model_name
        args.model_step = args.model2step[model_name]
        # if "receptive_field" in model_name:
        #     args.grid_size = 3
        for seed in [3]:
            i=0
            j=1
            ...
            network_pairs = f"{i}-{j}"
            selected_networks = network_pairs.split("-")
            args.ckpt_path = f"checkpoints/pickup_high_v6/{args.model_name}/seed{args.seed}/agent_{selected_networks[0]}_step_{args.model_step}.pt"
            args.ckpt_path2 = f"checkpoints/pickup_high_v6/{args.model_name}/seed{args.seed}/agent_{selected_networks[1]}_step_{args.model_step}.pt"
            args.saved_dir = f"logs/shortcut/{args.model_name}/{network_pairs}/{args.model_step}/seed{args.seed}/mode_{args.mode}"
