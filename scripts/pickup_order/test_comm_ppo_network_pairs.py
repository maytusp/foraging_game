...
class Args:
    ...
    # ckpt_path = f"checkpoints/pickup_order/{model_name}/{combination_name}/seed{seed}/agent_{selected_networks[0]}_step_{model_step}.pt"
    # ckpt_path2 = f"checkpoints/pickup_order/{model_name}/{combination_name}/seed{seed}/agent_{selected_networks[1]}_step_{model_step}.pt"
    # saved_dir = f"logs/pickup_order/{model_name}{network_pairs}/{combination_name}_{model_step}/seed{seed}/mode_{mode}"
    # if ablate_message:
...
if __name__ == "__main__":
    ...
    for i in range(args.num_networks):
        for j in range(i+1):
            ...
            network_pairs = f"{i}-{j}"
            selected_networks = network_pairs.split("-")
            args.ckpt_path = f"checkpoints/pickup_order/{args.model_name}/{args.combination_name}/seed{args.seed}/agent_{selected_networks[0]}_step_{args.model_step}.pt"
            args.ckpt_path2 = f"checkpoints/pickup_order/{args.model_name}/{args.combination_name}/seed{args.seed}/agent_{selected_networks[1]}_step_{args.model_step}.pt"
            args.saved_dir = f"logs/pickup_order/exp2/{args.model_name}/pair_{network_pairs}/{args.combination_name}_{args.model_step}/seed{args.seed}/mode_{args.mode}"
