...
class Args:
    ...
    wandb_project_name: str = "torch_temporalg_v2"
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(f"runs/temporalg_v2/{run_name}")
    writer.add_text(
        "hyperparameters",
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for i in range(num_agents):
            ...
            b_masks[i] = masks_stores[i].reshape(-1)
            ...
            for epoch in range(args.update_epochs):
                ...
                for start in range(0, args.num_envs, envsperbatch):
                    ...
                    mg_loss_element = mg_loss_element * mb_masks.float() # exclude duration where agents are silent
                    mg_loss = mg_loss_element.mean() # message gradient is computed only whn agents send messages
