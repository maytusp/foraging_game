...
if __name__ == "__main__":
    ...
    for i in range(args.num_networks):
        agents[i] = PPOLSTMCommAgent(num_actions=num_actions, num_channels=args.num_channels).to(device)
    ...
    for i in range(num_agents):
        ...
        obs[i] = torch.zeros((args.num_steps, args.num_envs, args.num_channels, args.num_obs_grid, args.num_obs_grid)).to(device) # obs: vision
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            for i in range(num_agents):
                ...
                agent_id = selected_agents[i] # Two embodied agents have chance to share the same neural networks during training (self-play)

                # ALGO LOGIC: action logic
                with torch.no_grad():
                    action[i], action_logprob[i], _, s_message[i], message_logprob[i], _, value[i], next_lstm_state[i] = agents[agent_id].get_action_and_value((next_obs[i], next_locs[i], next_r_messages[i]), 
        ...
        for i in range(num_agents):
            ...
            # flatten the batch
            b_obs[i] = obs[i].reshape((-1, args.num_channels, args.num_obs_grid, args.num_obs_grid))
