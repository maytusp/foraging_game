...
class PPOLSTMDIALBroadAgent(nn.Module):
    '''
    ...
    def __init__(self, num_actions, grid_size=10, max_energy=200, embedding_size=8, num_channels=1, n_words=10):
        ...
        self.actor = layer_init(nn.Linear(128, num_actions), std=0.01)
        self.critic = layer_init(nn.Linear(128, 1), std=1)
        self.message_head = nn.Linear(128, n_words)
    ...
    def get_action_and_value(self, input, lstm_state, done, past_action=None, past_message=None, tracks=None, train_mode=False):
        ...
        sampled_message = torch.argmax(message, dim=1)
        action_logits = self.actor(hiddens)
        action_probs = Categorical(logits=action_logits)
        action_pmf = nn.Softmax(dim=1)(action_logits)
        ...
        if past_message is not(None):
            message = past_message
        return action, action_probs.log_prob(action), action_probs.entropy(), message, message_probs.log_prob(sampled_message), message_probs.entropy(), self.critic(hiddens), lstm_state
    ...
    def get_message(self, hidden, train_mode=False):
        ...
        if train_mode:
            # DRU Operation: follows https://github.com/minqi/learning-to-communicate-pytorch/
            # Regularization based on RIAL/DIAL paper (Foerster et. al.)
            m_reg = message_logits + torch.randn(message_logits.size()).to(message_logits.device) * self.sigma
            message = torch.softmax(m_reg, -1)
            message_probs = None
        else:
            scale = 2*20
            message_probs = Categorical(logits=message_logits)
            message = torch.softmax(message_logits * scale, -1)
    ...
    def swap_message(self, s_message):
        ...
        s_message = s_message.view(-1, 2, self.n_words)  # Each row contains [a1, a2]
        print(f"s_message.shape {s_message.shape}")
        # Swap along dimension 1
        r_message = s_message.flip(dims=[1])  # Flip [a1, a2] -> [a2, a1]

        # Reshape back to (B, n_words)
        r_message = r_message.view(-1, self.n_words)
        return r_message