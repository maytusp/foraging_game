# 21 Mar 2025: Given N items (starting from 4), agents have to pick 2 highest-score items in chronologically orders.
                ...
                for food in self.foods:
                    if (self.l2_dist(food.position, agent.position) <= np.sqrt(2)):
                        ...
                        if food.strength_required - food.reduced_strength <= agent.strength and not food.carried:
                            ...
                            if food.id == self.pickup_order[curr_food_order] and food.id in self.target_food_ids:
