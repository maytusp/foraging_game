# Agents are spawn far away and can communicate when they are near each other
# Note: This is used until 7 Jan 2025, we move to more complex env.
# TODO Add item types as observations if the number of items >= 4
import pygame
import numpy as np
import random
import time
...
class Environment(ParallelEnv):
    ...
    def __init__(self, truncated=False, torch_order=True, num_agents=2, n_words=10, message_length=1, use_message=False, 
        ...
        self.single_observation_space = spaces.Dict(
            ...
            "is_m_sent": spaces.Box(0, 1, shape=(1,), dtype=np.int64), # is message sent successfully
    ...
    def observe(self):
        ...
        if len(self.possible_agents)==1:
            ...
            for i, agent in enumerate(self.agent_maps):
                ...
                agent_obs[i]['is_m_sent'] = self.check_comm_range()
    ...
    def check_comm_range(self):
        location1, location2 = self.agent_maps[0].position, self.agent_maps[1].position
        visible_range = self.image_size // 2 # visible_range(3x3) = 1, (5x5) = 2, (7x7) =3
        if (location1[0] >= location2[0] - visible_range and location1[0] <= location2[0] + visible_range and
            location1[1] >= location2[1] - visible_range and location1[1] <= location2[1] + visible_range):
            return 1
        else:
            return 0
