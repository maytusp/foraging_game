# Use first-meet condition (agents meet for the first time) instead of distance
# Limit number of times agents can send message
# Use the 0th token as a silence token and agents cannot produce this token
...
class EnvConfig:
    ...
    comm_steps: int = 5   # number of steps agents can send message
...
class TorchTemporalEnv:
    ...
    def __init__(self, cfg: EnvConfig, device: torch.device | str = "cuda", num_envs: int = 1):
        ...
        # Track when agent pairs first meet (-1 means never met)
        self.meet_step = torch.full((self.B, A, A), -1, dtype=torch.long, device=self.device)
    ...
    def observe(self):
        ...
        dist = torch.maximum((ay - by).abs(), (ax - bx).abs())  # (B,A,A)
