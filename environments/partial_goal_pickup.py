...
class Environment(ParallelEnv):
    ...
    def __init__(self, truncated=False, torch_order=True, num_agents=2, n_words=10, message_length=1, use_message=False, 
        ...
        self.N_val = N_val # number of possible values
        self.N_att = N_att # number of attributes
        ...
        self.single_observation_space = spaces.Dict(
            ...
            "goal": spaces.Box(0, self.N_val, shape=(self.N_att,), dtype=np.float32),
    ...
    def reset(self, seed=42, options=None):
        ...
        # print("mask", self.attribute_mask)
        self.foods = [Food(position=self.random_food_position(), 
                            food_type = food_id+1,
                            id=food_id,
                            identical_item_obs=self.identical_item_obs,
                            attribute=self.generated_attributes[food_id]) for food_id in range(self.N_i)
                    ]
    ...
    def observe(self):
        ...
        if len(self.possible_agents)==1:
            ...
            for i, agent in enumerate(self.agent_maps):
                ...
                switch_agent = {0:1, 1:0} # Switch agent: Agent0 sees Agent1's item attribute but does not see its own
                agent_obs[i]['goal'] = self.goal_attribute * self.attribute_mask[switch_agent[i]]
...
class EnvAgent:
    ...
    def observe(self, environment): #TODO Check this again
        ...
        attribute_mask = environment.attribute_mask[self.id]
                ...
                if 0 <= x < self.grid_size and 0 <= y < self.grid_size:
                    ...
                    if obj is None:
                        ...
                        if self.fully_visible: # If agent can see all attributes
                            ...
                            food_attribute_data[dx+ob_range, dy+ob_range] = attribute_mask * obj.attribute
...
class Food:
    def __init__(self, position, food_type, id, identical_item_obs, attribute):
        self.identical_item_obs = identical_item_obs
        self.position = position
        self.food_type = food_type
        self.strength_required = 6 # agent's strength is 3, it requires two agents to pickup.
