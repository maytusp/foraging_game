...
@dataclass
class Args:
    save_dir = "checkpoints/pickup_high_invisible/ppo_dial_broad"
    os.makedirs(save_dir, exist_ok=True)
    load_pretrained = False
    ckpt_path = ""
    save_frequency = int(1e5)
    # exp_name: str = os.path.basename(__file__)[: -len(".py")]
    exp_name = "ppo_dial_broad"
...
if __name__ == "__main__":
    ...
    agent = PPOLSTMDIALBroadAgent(num_actions=num_actions, num_channels=args.num_channels, n_words=args.n_words).to(device)
    ...
    r_messages = torch.zeros((args.num_steps, args.num_envs, args.n_words)).to(device) # obs: received message
    actions = torch.zeros((args.num_steps, args.num_envs)).to(device) # action: physical action
    s_messages = torch.zeros((args.num_steps, args.num_envs, args.n_words)).to(device) # action: sent message
