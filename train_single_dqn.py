...
#EVAL
VISUALIZE = False
VIDEO_SAVED_DIR = "vids/dqn_config1/"
...
def train_lstm_dql(env, num_episodes):
    wandb.init(
        ...
        project="train_single_dqn_foraging",
    ...
    for episode in range(num_episodes):
        ...
        # init hidden
        h = torch.randn(1, NUM_LSTM_LAYER, HIDDEN_DIM)
        c = torch.randn(1, NUM_LSTM_LAYER, HIDDEN_DIM)
        while not done and ep_step < 30:
            hidden = (h, c)
            action, message, hidden = agent.select_action(image, loc, hidden)
